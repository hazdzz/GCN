...
def get_parameters():
    ...
    parser.add_argument('--model', type=str, default='gcn', help='graph neural network model')
    ...
    parser.add_argument('--K', type=int, default=2, help='K layer, default as 2')
    ...
    model_save_path = model_name + '_' + gso_type + '_' + str(K) + '_layer' + '.pth'
    model_save_path = os.path.join(model_save_dir, model_save_path)

    return device, dataset, model_name, gso_type, lr, weight_decay, droprate, n_hid, enable_bias, K, epochs, opt, early_stopping_patience, model_save_path
...
def process_data(device, dataset, gso_type):
    ...
    filter = utility.calc_gso(adj, gso_type)
...
def prepare_model(n_feat, n_hid, n_class, enable_bias, K, droprate, \
                early_stopping_patience, model_save_path, opt, lr):
    model = models.GCN(n_feat, n_hid, n_class, enable_bias, K, droprate).to(device)
...
if __name__ == "__main__":
    device, dataset, model_name, gso_type, lr, weight_decay, droprate, n_hid, enable_bias, K, epochs, opt, early_stopping_patience, model_save_path = get_parameters()
    feature, filter, label, idx_train, idx_val, idx_test, n_feat, n_class = process_data(device, dataset, gso_type)
    model, loss, early_stopping, optimizer, scheduler = prepare_model(n_feat, n_hid, n_class, enable_bias, K, droprate, early_stopping_patience, model_save_path, opt, lr)
